...
suspend fun processMessage(
    ...
    when {
        ...
        msg.startsWith("ST300GPS;") ||
            ...
            saveOrUpdateGps(imei, gps, dailyTripService, msg)
        }

        else -> println("[$timestamp] ⚠️ Pacote desconhecido: ${msg.take(80)}...")
    }
}
...
fun parseGpsPacket(data: String): GpsData? = try {
    ...
    val latitude = parts.getOrNull(7)?.toDoubleOrNull()?.let {
        if (it in -90.0..90.0) it else { println("❌ Latitude inválida: $it"); return null }
    } ?: return null

    val longitude = parts.getOrNull(8)?.toDoubleOrNull()?.let {
        if (it in -180.0..180.0) it else { println("❌ Longitude inválida: $it"); return null }
    } ?: return null
    ...
    val gpsFixed       = parts.getOrNull(12)?.toIntOrNull() == 1
    ...
    GpsData(
        ...
        speed          = speed,
        heading        = heading,
        ignition       = validateIgnition(statusBits),
...
fun validateIgnition(statusBits: String?): Boolean = when {
    statusBits.isNullOrBlank()              -> false
    statusBits.length != 6                  -> { println("⚠️ StatusBits tamanho inválido: '$statusBits'"); false }
    !statusBits.all { it in '0'..'1' }      -> { println("⚠️ StatusBits com caracteres inválidos: '$statusBits'"); false }
    else                                    -> statusBits[0] == '1'
}
...
fun parseDeviceDateTime(dateStr: String?, timeStr: String?): LocalDateTime? {
    return try {
        ...
        LocalDateTime(year, month, day, h, m, s)

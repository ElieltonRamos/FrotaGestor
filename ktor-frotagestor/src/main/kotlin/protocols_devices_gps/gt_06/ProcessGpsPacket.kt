...
// ─── Alarm Types ──────────────────────────────────────────────────────────────
// alarmType=4 observado para ACC ON e ACC OFF — sem distinção entre os dois nos logs
...
suspend fun processPacketGT06(
    ...
    return when (protocolNo) {
        ...
        GT06Protocol.GPS       -> handleGps(content, serialNumber, currentImei, "GPS", dailyTripService)
        GT06Protocol.GPS_LBS   -> handleGps(content, serialNumber, currentImei, "GPS_LBS", dailyTripService)
        GT06Protocol.ALARM     -> handleAlarm(content, serialNumber, currentImei, dailyTripService)
        GT06Protocol.GT06_EXT  -> handleGT06Ext(content, currentImei, dailyTripService)
        GT06Protocol.CMD_REPLY -> handleCommandReply(content, currentImei)
        else -> {
            println("[${now()}] ⚠️ Protocol desconhecido: 0x${"%02X".format(protocolNo)}")
            GT06ProcessResult()
        }
    }
}
...
// Ignição: bit 1 do terminalInfo — confirmado empiricamente (0x44=off, 0x46=on)
private suspend fun handleHeartbeat(content: ByteArray, serialNumber: Int, imei: String?): GT06ProcessResult {
    ...
    println("[${now()}] ❤️ HEARTBEAT | IMEI=$imei | ign=$ignition | gsm=$gsmSignal")
...
// ignitionBit/terminalInfoOffset não validados com logs reais — usando defaults do padrão GT06
private suspend fun handleGps(
    ...
    if (imei == null) {
        println("[${now()}] ❌ GPS recebido sem IMEI registrado")
        return GT06ProcessResult()
    }

    val gps = parseGpsContent(content, imei, type, serialNumber) ?: return GT06ProcessResult()
    println("[${now()}] 📍 ${gps.toCompactLog()}")

    saveOrUpdateGps(imei, gps, dailyTripService)
    return GT06ProcessResult()
}

// ─── ALARM (0x16) ─────────────────────────────────────────────────────────────
// Layout real confirmado empiricamente:
// [26] = byte fixo (0x0B) — ignorado
// [27] = terminalInfo — bit 1 = ignição (0x44=off, 0x46=on, mesmo encoding do heartbeat)
// [28] = gsmSignal
// [29] = alarmType
private suspend fun handleAlarm(
    ...
    if (imei == null) {
        println("[${now()}] ❌ ALARM recebido sem IMEI registrado")
        return GT06ProcessResult()
    }
    ...
    println("[${now()}] 🚨 ALARM | ${gps.toCompactLog()}")
...
// ignitionBit=1, terminalInfoOffset=27: assumido igual ao 0x16 — aguardando validação
private suspend fun handleGT06Ext(
    content:          ByteArray,
    currentImei:      String?,
    dailyTripService: DailyTripService
): GT06ProcessResult {
    if (content.size < 8) {
        println("[${now()}] ❌ GT06_EXT packet muito curto")
        return GT06ProcessResult()
    }

    val imei         = decodeBcdImei(content.copyOfRange(0, 8))
    val resolvedImei = currentImei ?: run {
        println("[${now()}] ℹ️ GT06_EXT — IMEI extraído do pacote: $imei")
        imei
    }
    ...
    println("[${now()}] 📍 GT06_EXT | ${gps.toCompactLog()}")
    logAlarmDebug(gpsContent)

    saveOrUpdateGps(resolvedImei, gps, dailyTripService)
...
private fun handleCommandReply(content: ByteArray, imei: String?): GT06ProcessResult {
    ...
    println("[${now()}] 📨 CMD REPLY | IMEI=$imei | $text")
    return GT06ProcessResult()
}
 ...
 * [15]    Speed (km/h)
 * [16]    Course byte1: bit4=gpsFixed, bit3=West, bit2=isNorth (0=South), bit1-0=heading high
 * [17]    Course byte2: heading low
 * [18-19] MCC
 * [20]    MNC
 * [21-22] LAC
 * [23-25] Cell ID (3 bytes)
 * [26]    byte fixo no 0x16/GT06_EXT (ignorado) / terminalInfo no 0x10/0x12
 * [27]    terminalInfo no 0x16/GT06_EXT — bit 1 = ignição
 * [28]    gsmSignal
 * [29]    alarmType
 *
 * terminalInfoOffset: 26 para 0x10/0x12, 27 para 0x16/GT06_EXT
 * courseByte1 bit2: 0=South, 1=North — confirmado empiricamente (Brasil retorna 0, lat negativa)
 */
fun parseGpsContent(
    content:            ByteArray,
    imei:               String,
    ...
    return try {
        ...
        val isSouth  = (courseByte1 shr 2) and 0x01 == 0  // bit2: 0=South, 1=North
...
// ─── DB ───────────────────────────────────────────────────────────────────────
suspend fun saveOrUpdateGps(imei: String, gps: GpsData, dailyTripService: DailyTripService) {

...
object GpsDevicesTable : Table("gps_devices") {
    ...
    val vehicleId = integer("vehicle_id").references(VehiclesTable.id).nullable() // ALTERADO: agora aceita NULL
    val imei = varchar("imei", 50).uniqueIndex()
    val latitude = decimal("latitude", 9, 6).default(0.toBigDecimal()) // ALTERADO: valor padrão
    val longitude = decimal("longitude", 9, 6).default(0.toBigDecimal()) // ALTERADO: valor padrão

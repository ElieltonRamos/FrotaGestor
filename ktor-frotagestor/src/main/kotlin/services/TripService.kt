...
class TripService {
    ...
    suspend fun getAllTrips(
        ...
        return DatabaseFactory.dbQuery {
            val query = TripsTable
                // ✅ CORRIGIR: LEFT JOIN para incluir viagens sem motorista
                .join(DriversTable, JoinType.LEFT, additionalConstraint = { TripsTable.driverId eq DriversTable.id })
                ...
                .apply {
                    ...
                    if (!driverNameFilter.isNullOrBlank()) {
                        andWhere { DriversTable.name like "%${driverNameFilter}%" }
                    }
                    if (!vehiclePlateFilter.isNullOrBlank()) {
            ...
            val results = query
                ...
                .map {
                    Trip(
                        ...
                        // ✅ Coordenadas GPS
                        startLatitude = it[TripsTable.startLatitude]?.toDouble(),
                        startLongitude = it[TripsTable.startLongitude]?.toDouble(),
                        endLatitude = it[TripsTable.endLatitude]?.toDouble(),
                        endLongitude = it[TripsTable.endLongitude]?.toDouble(),

                        // Horários
                        startTime = it[TripsTable.startTime],
                        endTime = it[TripsTable.endTime],

                        // ✅ Distância (campo já existia)
                        distanceKm = it[TripsTable.distanceKm]?.toDouble(),

                        // ✅ Automação
                        autoGenerated = it[TripsTable.autoGenerated],
                        gpsDeviceId = it[TripsTable.gpsDeviceId],

                        // ✅ Velocidades
                        maxSpeedKmh = it[TripsTable.maxSpeedKmh]?.toDouble(),
                        avgSpeedKmh = it[TripsTable.avgSpeedKmh]?.toDouble(),

                        // Status
                        status = it[TripsTable.status],

                        // ✅ JOIN fields - usar getOrNull por causa do LEFT JOIN
                        driverName = it.getOrNull(DriversTable.name),
    ...
    suspend fun findTripById(id: Int): ServiceResponse<Any> {
        val trip = DatabaseFactory.dbQuery {
            TripsTable
                // ✅ LEFT JOIN para incluir viagens sem motorista
                .join(DriversTable, JoinType.LEFT, additionalConstraint = { TripsTable.driverId eq DriversTable.id })
                .join(VehiclesTable, JoinType.INNER, additionalConstraint = { TripsTable.vehicleId eq VehiclesTable.id })
                ...
                ?.let {
                    Trip(
                        ...
                        // ✅ JOIN fields - getOrNull por causa do LEFT JOIN
                        driverName = it.getOrNull(DriversTable.name),
                        vehiclePlate = it[VehiclesTable.plate]
                    )
                }
    ...
    suspend fun getTripsBySubfleet(
        subfleetId: Int,
        page: Int = 1,
        limit: Int = 10,
        sortBy: Column<*> = TripsTable.id,
